    ...
    document.querySelectorAll('.box').forEach(image => {
        ...
        if (image.innerText.length !== 0) {
            if (image.innerText == 'Wpawn' || image.innerText == 'Bpawn') {
                image.innerHTML = `${image.innerText} <img class='allimg allpawn' src="/static/resources/images/pecas/${image.innerText}.png" alt="">`
                ...
                image.innerHTML = `${image.innerText} <img class='allimg' src="/static/resources/images/pecas/${image.innerText}.png" alt="">`
                image.style.cursor = 'pointer'
            }
        }
    })
...
function coloring() {
    ...
    color.forEach(color => {
        ...
        if (a % 2 !== 0) {
            color.style.backgroundColor = 'rgb(100, 49, 43)'
        }
        // if (a % 2 == 0) {
...
document.querySelectorAll('.box').forEach(item => {
    ...
    item.addEventListener('click', function () {
        ...
        if (tog % 2 !== 0) {
            document.getElementById('tog').innerText = "Turno das Brancas"
            whosTurn('W')
        }
        if (tog % 2 == 0) {
            document.getElementById('tog').innerText = "Turno das Vermelhas"
            whosTurn('B')
        }
...
exitButton.onclick = function() {
    window.close(); // Attempt to close the window (may not work in all browsers)
}
